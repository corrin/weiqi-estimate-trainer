...
export default function GoogleSignIn() {
  ...
  const handleGoogleSignIn = () => {
    if (!window.google) {
      alert('Google Sign-In is not available right now. Please try again.')
      return
    }
    ...
    <div className="flex justify-center">
      <button
        ...
        className="flex items-center gap-3 px-6 py-3 rounded-xl bg-white text-stone-800 font-medium
          hover:bg-stone-100 transition-all shadow-lg active:scale-[0.98] border border-stone-200"
      >
      ...
      <div id="google-btn-fallback" className="hidden" />
    </div>
  )
}
